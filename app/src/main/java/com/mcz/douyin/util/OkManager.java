...
/**
 * Created by CC on 2023/9/16 0015.
 * 封装工具类
...
public class OkManager {
    ...
    public void sendComplexForm(String url, String header, final Fun4 callback) {
        FormBody.Builder form_builder = new FormBody.Builder();  //表单对象，包含以input开始的对象，模拟一个表单操作，以HTML表单为主
        //如果键值对不为空，且值不为空
        ...
        RequestBody request_body = form_builder.build();
        //采用post的方式进行提交
        Request request = new Request.Builder().url(url).post(request_body).addHeader("token", header).build();
        client.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(Call call, IOException e) {
                Log.e("3333", "fff");
